...
// KeepAliveMode controls how HTTP/1.1 keep-alive behavior is emulated for fingerprinting.
type KeepAliveMode int

const (
	// KeepAliveModeDefault uses standard Go HTTP/1.1 keep-alive behavior.
	KeepAliveModeDefault KeepAliveMode = iota
	// KeepAliveModeChrome emulates Chrome's keep-alive behavior.
	// Chrome sends "Connection: keep-alive" on HTTP/1.1 and uses ~300s timeout.
	KeepAliveModeChrome
	// KeepAliveModeFirefox emulates Firefox's keep-alive behavior.
	// Firefox uses slightly different timeout values.
	KeepAliveModeFirefox
	// KeepAliveModeDisabled disables keep-alive entirely (Connection: close).
	KeepAliveModeDisabled
	// KeepAliveModeCustom allows custom keep-alive configuration via
	// KeepAliveTimeout and KeepAliveMaxRequests fields.
	KeepAliveModeCustom
)

// ConnectionReuseMode controls how HTTP/1.1 connections are pooled and reused.
type ConnectionReuseMode int

const (
	// ConnectionReuseModeDefault uses standard Go connection pooling.
	ConnectionReuseModeDefault ConnectionReuseMode = iota
	// ConnectionReuseModeChrome emulates Chrome's connection reuse patterns.
	// Chrome aggressively reuses connections and preemptively opens them.
	ConnectionReuseModeChrome
	// ConnectionReuseModeFirefox emulates Firefox's connection reuse patterns.
	ConnectionReuseModeFirefox
	// ConnectionReuseModeSingle uses a single connection per request (no pooling).
	ConnectionReuseModeSingle
	// ConnectionReuseModeCustom allows custom connection reuse via
	// MaxConnectionAge and other fields.
	ConnectionReuseModeCustom
)
...
// To explicitly enable HTTP/2 on a transport, use github.com/sardanioss/net/http2
...
type Transport struct {
	...
	// KeepAliveMode controls how HTTP/1.1 keep-alive behavior is emulated.
	// Default is KeepAliveModeDefault which uses standard Go behavior.
	KeepAliveMode KeepAliveMode

	// KeepAliveTimeout is the timeout value to send in Keep-Alive header.
	// Only used when KeepAliveMode is KeepAliveModeCustom.
	// Chrome typically uses 300 seconds.
	KeepAliveTimeout time.Duration

	// KeepAliveMaxRequests is the max requests to send in Keep-Alive header.
	// Only used when KeepAliveMode is KeepAliveModeCustom.
	// Set to 0 to omit max from the header.
	KeepAliveMaxRequests int

	// SendKeepAliveHeader controls whether to explicitly send the Keep-Alive
	// header on HTTP/1.1 connections. Some browsers always send it, others don't.
	SendKeepAliveHeader bool

	// ConnectionReuseMode controls how connections are reused across requests.
	// This affects fingerprinting by controlling connection pooling behavior.
	ConnectionReuseMode ConnectionReuseMode

	// ConnectionPrewarm enables pre-establishing connections before they're needed.
	// Chrome preemptively opens connections to likely-needed hosts.
	ConnectionPrewarm bool

	// MaxConnectionAge limits how long a connection can be reused.
	// Set to 0 for no limit. Chrome typically reuses connections for a few minutes.
	MaxConnectionAge time.Duration

	// ForceHTTP10 forces HTTP/1.0 instead of HTTP/1.1 for fingerprinting purposes.
	// Some legacy systems or specific fingerprints require HTTP/1.0.
	ForceHTTP10 bool
}
...
func (t *Transport) Clone() *Transport {
	...
	t2 := &Transport{
		...
		// HTTP/1.1 Fingerprinting fields
		KeepAliveMode:        t.KeepAliveMode,
		KeepAliveTimeout:     t.KeepAliveTimeout,
		KeepAliveMaxRequests: t.KeepAliveMaxRequests,
		SendKeepAliveHeader:  t.SendKeepAliveHeader,
		ConnectionReuseMode:  t.ConnectionReuseMode,
		ConnectionPrewarm:    t.ConnectionPrewarm,
		MaxConnectionAge:     t.MaxConnectionAge,
		ForceHTTP10:          t.ForceHTTP10,
		// HTTP/2 fingerprinting fields
		PseudoHeaderOrder: t.PseudoHeaderOrder,
...
// GetKeepAliveHeader returns the Keep-Alive header value based on the transport's
// keep-alive configuration. Returns empty string if keep-alive header should not be sent.
func (t *Transport) GetKeepAliveHeader() string {
	switch t.KeepAliveMode {
	case KeepAliveModeChrome:
		return "timeout=300"
	case KeepAliveModeFirefox:
		return "timeout=115"
	case KeepAliveModeCustom:
		if t.KeepAliveTimeout > 0 {
			timeout := int(t.KeepAliveTimeout.Seconds())
			if t.KeepAliveMaxRequests > 0 {
				return fmt.Sprintf("timeout=%d, max=%d", timeout, t.KeepAliveMaxRequests)
			}
			return fmt.Sprintf("timeout=%d", timeout)
		}
	case KeepAliveModeDisabled:
		return ""
	}
	return ""
}

// GetConnectionHeader returns the Connection header value based on keep-alive mode.
func (t *Transport) GetConnectionHeader() string {
	switch t.KeepAliveMode {
	case KeepAliveModeDisabled:
		return "close"
	case KeepAliveModeChrome, KeepAliveModeFirefox, KeepAliveModeCustom:
		return "keep-alive"
	default:
		// Default behavior: HTTP/1.1 implicitly uses keep-alive
		return ""
	}
}

// ShouldDisableKeepAlives returns whether keep-alives should be disabled.
func (t *Transport) ShouldDisableKeepAlives() bool {
	if t.DisableKeepAlives {
		return true
	}
	return t.KeepAliveMode == KeepAliveModeDisabled
}

// GetEffectiveIdleConnTimeout returns the effective idle connection timeout
// based on keep-alive mode.
func (t *Transport) GetEffectiveIdleConnTimeout() time.Duration {
	...
	return 90 * time.Second // Default
}

// GetEffectiveMaxIdleConnsPerHost returns the effective max idle connections
// per host based on connection reuse mode.
func (t *Transport) GetEffectiveMaxIdleConnsPerHost() int {
	if t.MaxIdleConnsPerHost > 0 {
		return t.MaxIdleConnsPerHost
	}
	switch t.ConnectionReuseMode {
	case ConnectionReuseModeChrome:
		return 6 // Chrome typically allows 6 connections per host
	case ConnectionReuseModeFirefox:
		return 6 // Firefox also uses 6
	case ConnectionReuseModeSingle:
		return 1
	}
...
func (t *Transport) onceSetNextProtoDefaults() {
	...
	// github.com/sardanioss/net/http2 instead of the bundled copy, try to
